using AuthFoundation.Common;
using AuthFoundation.Contracts;
using AuthFoundation.Services;
using Microsoft.AspNetCore.Mvc;

namespace AuthFoundation.Controllers.Auth;

[ApiController]
[Route("revoke")]
public sealed class RevokeController : ControllerBase
{
    private readonly IOidcStore _store;

    public RevokeController(IOidcStore store)
    {
        _store = store;
    }

    [HttpPost]
    public async Task<IActionResult> Post()
    {
        try
        {
            IFormCollection form = await Request.ReadFormAsync();
            var input = new RevokeTokenRequest(form["token"].ToString());
            ValidateUtil.IndispensableParam(input.Token, "token");
            _store.RevokeAccessToken(input.Token);
            return Ok(new ResultOutput("revoked"));
        }
        catch (ApiException ex)
        {
            return new ObjectResult(new ErrorOutput(ex)) { StatusCode = (int)ex.StatusCode };
        }
    }
}
