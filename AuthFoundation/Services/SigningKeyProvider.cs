...
public sealed class SigningKeyProvider : IDisposable
{
    ...
    {
        if (!AppConfig.IsSigningKeyConfigured())
        {
    ...
    private static SigningKeyProvider GenerateEphemeral()
    {
        RSA rsa = RSA.Create(2048);
        return new SigningKeyProvider($"local-ephemeral-{Helper.GenerateHex(8)}", rsa);
    }
