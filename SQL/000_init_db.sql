...
;WITH TargetTables AS (
    SELECT OBJECT_ID(N'[auth].[agent_audit_log]') AS object_id
    UNION ALL SELECT OBJECT_ID(N'[auth].[agent_delegation]')
    UNION ALL SELECT OBJECT_ID(N'[auth].[agent_master]')
    UNION ALL SELECT OBJECT_ID(N'[auth].[user_info]')
    ...
    UNION ALL SELECT OBJECT_ID(N'[auth].[jwk_master]')
...
DROP TABLE IF EXISTS [auth].[agent_audit_log];
DROP TABLE IF EXISTS [auth].[agent_delegation];
DROP TABLE IF EXISTS [auth].[agent_master];
...
DROP TABLE IF EXISTS [auth].[jwk_master];
